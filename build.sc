...
object `qasrl-site` extends CommonMainModule with ScalatexBuild.ScalatexModule {
  ...
  def generate() = T.command {
    ...
    val demoJSPath = `qasrl-demo`.fastOpt().path.toString
    val demoJSDepsPath = `qasrl-demo`.aggregatedJSDeps().path.toString
    val run = runFn()
    run(Seq(browserJSPath, browserJSDepsPath, demoJSPath, demoJSDepsPath))
  }
}
...
trait QASRLDemoModule extends CommonMainModule {
  def millSourcePath = build.millSourcePath / "qasrl-demo"
}
object `qasrl-demo` extends CommonMainModule with ScalaJSModule with SimpleJSDepsBuild.SimpleJSDeps {
  def scalaJSVersion = T(thisScalaJSVersion)
  ...
  def mainClass = T(Some("qasrl.demo.Main"))
